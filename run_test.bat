...
@REM call mvn exec:java -Dexec.mainClass="tarmorn.TLearn" > "out\%dataset%\run.log" 2>&1
...
call python eval.py --dataset %dataset% --rules out/%dataset%/rule.txt --ranking_file out/%dataset%/eval.txt --aggregation_function noisyor --noisyor_positive_method matching2 > out/%dataset%/eval-noisyor+matching2.log
call python eval.py --dataset %dataset% --rules out/%dataset%/rule.txt --ranking_file out/%dataset%/eval.txt --aggregation_function noisyor --noisyor_negative_method prune > out/%dataset%/eval-noisyor-prune.log
call python eval.py --dataset %dataset% --rules out/%dataset%/rule.txt --ranking_file out/%dataset%/eval.txt --aggregation_function noisyor --noisyor_negative_method cluster > out/%dataset%/eval-noisyor-cluster.log
call python eval.py --dataset %dataset% --rules out/%dataset%/rule.txt --ranking_file out/%dataset%/eval.txt --aggregation_function noisyor --noisyor_negative_method prune --noisyor_positive_method mst > out/%dataset%/eval-noisyor-prune+mst.log
