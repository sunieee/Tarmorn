...
//            println(R2supp)
            ...
            // 保存所有LSH bucket到JSON文件
            saveLSHBucketsToJson()
...
//        if (myAtom.isL2Atom && !myAtom.isBinary) {  这种写法有问题，会漏掉L1Atom的情况
     ...
     * 保存所有LSH bucket到JSON文件
     */
    private fun saveLSHBucketsToJson() {
        val outDir = File("out/" + Settings.DATASET)
        outDir.mkdirs() // 确保out目录存在

        val outputFile = File(outDir, "lsh_buckets.json")
        println("Saving LSH buckets to ${outputFile.absolutePath}...")
        ...
        BufferedWriter(FileWriter(outputFile)).use { writer ->
            writer.write("{\n")
            writer.write("  \"totalBuckets\": ${key2headAtomSnapshot.size},\n")
            writer.write("  \"totalAtoms\": ${key2headAtomSnapshot.values.sumOf { it.size }},\n")
            writer.write("  \"buckets\": [\n")

            val bucketEntries = key2headAtomSnapshot.entries.toList()
            bucketEntries.forEachIndexed { index, (key, atoms) ->
                writer.write("    {\n")
                writer.write("      \"bucketKey\": $key,\n")
                writer.write("      \"atomCount\": ${atoms.size},\n")
                writer.write("      \"atoms\": [\n")

                atoms.forEachIndexed { atomIndex, atom ->
                    // 获取atom的详细信息
                    val relationPath = atom.relationId
                    val entityId = atom.entityId
                    val relationName = IdManager.getRelationString(relationPath)
                    
                    // 判断原子类型
                    val atomType = when {
                        entityId == IdManager.getYId() -> "Binary"
                        entityId == IdManager.getXId() -> "Loop"
                        entityId == 0 -> "Existence"
                        entityId > 0 -> "Constant"
                        else -> "Unknown"
                    }
                    ...
                    // 获取entity名称（如果适用）
                    val entityName = when {
                        entityId > 0 -> IdManager.getEntityString(entityId)
                        else -> null
                    }

                    writer.write("        {\n")
                    writer.write("          \"atom\": \"${atom.toString().replace("\"", "\\\"")}\",\n")
                    writer.write("          \"relationId\": $relationPath,\n")
                    writer.write("          \"relationName\": \"${relationName.replace("\"", "\\\"")}\",\n")
                    writer.write("          \"entityId\": $entityId,\n")
                    if (entityName != null) {
                        writer.write("          \"entityName\": \"${entityName.replace("\"", "\\\"")}\",\n")
                    }
                    writer.write("          \"atomType\": \"$atomType\"\n")
                    writer.write("        }")
                    if (atomIndex < atoms.size - 1) writer.write(",")
                    writer.write("\n")
                }

                writer.write("      ]\n")
                writer.write("    }")
                if (index < bucketEntries.size - 1) writer.write(",")
                writer.write("\n")
            }
        ...
        println("LSH buckets saved successfully to ${outputFile.absolutePath}")
        
        // 额外检查：查找特定关系的bucket分布
        checkSpecificRelationBuckets()
    }

    /**
     * 检查特定关系的LSH bucket分布情况
     */
    private fun checkSpecificRelationBuckets() {
        println("\n=== Checking specific relations bucket distribution ===")
        
        // 你提到的两个关系
        val relation1Name = "/location/country/capital"
        val relation2Name = "/location/location/contains"
        
        // 根据名称查找关系ID
        val relation1Id = IdManager.getRelationId(relation1Name)
        val relation2Id = IdManager.getRelationId(relation2Name)
        
        if (relation1Id == null || relation2Id == null) {
            println("Warning: Cannot find relation IDs for:")
            if (relation1Id == null) println("  - $relation1Name")
            if (relation2Id == null) println("  - $relation2Name")
            return
        }
        
        // 创建Binary原子
        val atom1 = MyAtom(relation1Id, IdManager.getYId())
        val atom2 = MyAtom(relation2Id, IdManager.getYId())
        
        println("Relation 1: $relation1Name")
        println("  Atom: $atom1")
        println("  isHeadAtom: ${atom1.isHeadAtom}")
        println("  isL1Atom: ${atom1.isL1Atom}")
        println("  isInverseRelation: ${IdManager.isInverseRelation(relation1Id)}")
        println("Relation 2: $relation2Name")
        println("  Atom: $atom2")
        println("  isHeadAtom: ${atom2.isHeadAtom}")
        println("  isL1Atom: ${atom2.isL1Atom}")
        println("  isInverseRelation: ${IdManager.isInverseRelation(relation2Id)}")
        
        // 检查这两个原子的MinHash签名和bucket分布
        val signature1 = minHashRegistry[Formula(atom1 = atom1)]
        val signature2 = minHashRegistry[Formula(atom1 = atom2)]
        
        if (signature1 == null || signature2 == null) {
            println("Warning: Cannot find MinHash signatures for these atoms")
            if (signature1 == null) println("  - $atom1 not in registry")
            if (signature2 == null) println("  - $atom2 not in registry")
            return
        }
        
        println("\nMinHash signatures:")
        println("Atom1 first 10 hashes: ${signature1.take(10).joinToString(", ")}")
        println("Atom2 first 10 hashes: ${signature2.take(10).joinToString(", ")}")
        
        // 检查它们在哪些bucket中
        val atom1Buckets = mutableSetOf<Int>()
        val atom2Buckets = mutableSetOf<Int>()
        val sharedBuckets = mutableSetOf<Int>()
        
        for (bandIndex in 0 until BANDS) {
            val key1 = signature1[bandIndex]
            val key2 = signature2[bandIndex]
            
            atom1Buckets.add(key1)
            atom2Buckets.add(key2)
            
            if (key1 == key2) {
                sharedBuckets.add(key1)
            }
        }
        
        println("\nBucket distribution:")
        println("Atom1 appears in ${atom1Buckets.size} unique buckets (out of $BANDS bands)")
        println("Atom2 appears in ${atom2Buckets.size} unique buckets (out of $BANDS bands)")
        println("Shared buckets: ${sharedBuckets.size}")
        
        if (sharedBuckets.isNotEmpty()) {
            println("\nShared bucket keys: ${sharedBuckets.take(10).joinToString(", ")}${if (sharedBuckets.size > 10) "..." else ""}")
            
            // 检查实际的bucket内容
            println("\nChecking actual bucket contents:")
            sharedBuckets.take(5).forEach { bucketKey ->
                val bucket = key2headAtom[bucketKey]
                if (bucket != null) {
                    val hasAtom1 = bucket.any { it == atom1 }
                    val hasAtom2 = bucket.any { it == atom2 }
                    println("  Bucket $bucketKey: size=${bucket.size}, hasAtom1=$hasAtom1, hasAtom2=$hasAtom2")
                }
            }
            
            // 现在检查为什么规则没有被创建
            println("\n=== Analyzing why rule was not created ===")
            
            // 获取实际的instance sets
            val instances1 = r2instanceSet[relation1Id]
            val instances2 = r2instanceSet[relation2Id]
            
            if (instances1 == null || instances2 == null) {
                println("ERROR: Instance sets not found!")
                if (instances1 == null) println("  - instances1 is null for relation1")
                if (instances2 == null) println("  - instances2 is null for relation2")
                return
            }
            
            println("\nInstance set sizes:")
            println("  Atom1 (${relation1Name}): ${instances1.size}")
            println("  Atom2 (${relation2Name}): ${instances2.size}")
            
            // 计算实际的交集和Jaccard
            val intersection = instances1.intersect(instances2)
            val union = instances1.union(instances2)
            val actualJaccard = if (union.size > 0) intersection.size.toDouble() / union.size else 0.0
            
            println("\nActual similarity metrics:")
            println("  Intersection size: ${intersection.size}")
            println("  Union size: ${union.size}")
            println("  Actual Jaccard: $actualJaccard")
            
            // 计算估计的Jaccard（基于MinHash共享bucket数）
            val estimatedJaccard = sharedBuckets.size.toDouble() / BANDS
            println("  Estimated Jaccard (from LSH): $estimatedJaccard (${sharedBuckets.size}/$BANDS)")
            
            // 估计交集大小
            val estimatedIntersection = estimateIntersectionSize(estimatedJaccard, instances1.size, instances2.size)
            println("  Estimated intersection size: $estimatedIntersection")
            
            // 模拟validateAndCreateFormula的逻辑
            println("\n=== Simulating validation logic ===")
            
            // 方向1: atom1 as head, atom2 as body
            println("\nDirection 1: ${atom1} <= ${atom2}")
            val metric1 = Metric(estimatedJaccard, estimatedIntersection, instances1.size, instances2.size)
            println("  Estimated Metric: $metric1")
            println("  estimateValid: ${metric1.estimateValid}")
            println("    - support >= MIN_SUPP * ESTIMATE_RATIO: ${estimatedIntersection} >= ${MIN_SUPP * ESTIMATE_RATIO} = ${estimatedIntersection >= MIN_SUPP * ESTIMATE_RATIO}")
            println("    - confidence > MIN_CONF * ESTIMATE_RATIO: ${metric1.confidence} > ${MIN_CONF * ESTIMATE_RATIO} = ${metric1.confidence > MIN_CONF * ESTIMATE_RATIO}")
            
            if (metric1.estimateValid) {
                println("  Would call validateAndCreateFormula...")
                val actualMetric1 = Metric(actualJaccard, intersection.size.toDouble(), instances1.size, instances2.size)
                println("  Actual Metric after validation: $actualMetric1")
                println("  valid: ${actualMetric1.valid}")
                println("    - support >= MIN_SUPP: ${intersection.size} >= $MIN_SUPP = ${intersection.size >= MIN_SUPP}")
                println("    - confidence > MIN_CONF: ${actualMetric1.confidence} > $MIN_CONF = ${actualMetric1.confidence > MIN_CONF}")
                
                if (actualMetric1.valid) {
                    println("  ✓ Rule SHOULD be created!")
                    
                    // 检查是否在atom2formula2metric中
                    // 规则存储格式：atom2formula2metric[bodyAtom][Formula(headAtom, bodyAtom)]
                    // 所以对于规则 atom1 <= atom2，应该在 atom2formula2metric[atom2] 中查找
                    println("  Checking in atom2formula2metric[bodyAtom=$atom2]...")
                    val formula2metric = atom2formula2metric[atom2]
                    if (formula2metric != null) {
                        println("    Found ${formula2metric.size} formulas with body=$atom2")
                        val existingFormula = formula2metric.keys.find { 
                            it.atom1 == atom1 && it.atom2 == atom2 
                        }
                        if (existingFormula != null) {
                            val storedMetric = formula2metric[existingFormula]!!
                            println("  ✓ Rule EXISTS in atom2formula2metric[bodyAtom]: $storedMetric")
                            println("    Formula: head=${existingFormula.atom1}, body=${existingFormula.atom2}")
                            
                            // 检查是否会被保存到文件
                            val allFormulas = formula2metric.entries.toList()
                                .sortedByDescending { it.value.confidence }
                            val highConfidenceFormulas = allFormulas.filter { it.value.confidence >= 0.6 }
                            val saved = if (highConfidenceFormulas.size >= 20) {
                                highConfidenceFormulas
                            } else {
                                allFormulas.take(20)
                            }
                            
                            val wouldBeSaved = saved.any { it.key == existingFormula }
                            val rank = allFormulas.indexOfFirst { it.key == existingFormula } + 1
                            
                            println("\n  === File Save Analysis ===")
                            println("    Total formulas with body=$atom2: ${allFormulas.size}")
                            println("    High confidence (>=0.6) formulas: ${highConfidenceFormulas.size}")
                            println("    This rule's confidence: ${storedMetric.confidence}")
                            println("    This rule's rank (by confidence): $rank / ${allFormulas.size}")
                            println("    Would be saved to file: ${if (wouldBeSaved) "✓ YES" else "✗ NO (filtered out)"}")
                            
                            if (!wouldBeSaved) {
                                println("\n  ⚠️ FOUND THE PROBLEM!")
                                println("    The rule EXISTS in memory but is FILTERED OUT when saving to file!")
                                println("    Reason: confidence ${storedMetric.confidence} < 0.6 and rank $rank > 20")
                            }
                        } else {
                            println("  ✗ Rule NOT FOUND in atom2formula2metric[bodyAtom=$atom2]")
                            println("    Looking for: head=$atom1, body=$atom2")
                            println("    Available formulas (first 5):")
                            formula2metric.entries.take(5).forEach { (f, m) ->
                                println("      head=${f.atom1}, body=${f.atom2}, metric=$m")
                            }
                        }
                    } else {
                        println("  ✗ No formulas found with body atom $atom2")
                    }
                } else {
                    println("  ✗ Rule would NOT pass final validation")
                }
            } else {
                println("  ✗ Would NOT call validateAndCreateFormula (estimateValid=false)")
            }
            
            // 方向2: atom2 as head, atom1 as body
            println("\nDirection 2: ${atom2} <= ${atom1}")
            val metric2 = Metric(estimatedJaccard, estimatedIntersection, instances2.size, instances1.size)
            println("  Estimated Metric: $metric2")
            println("  estimateValid: ${metric2.estimateValid}")
            println("    - support >= MIN_SUPP * ESTIMATE_RATIO: ${estimatedIntersection} >= ${MIN_SUPP * ESTIMATE_RATIO} = ${estimatedIntersection >= MIN_SUPP * ESTIMATE_RATIO}")
            println("    - confidence > MIN_CONF * ESTIMATE_RATIO: ${metric2.confidence} > ${MIN_CONF * ESTIMATE_RATIO} = ${metric2.confidence > MIN_CONF * ESTIMATE_RATIO}")
            
            if (metric2.estimateValid) {
                println("  Would call validateAndCreateFormula...")
                val actualMetric2 = Metric(actualJaccard, intersection.size.toDouble(), instances2.size, instances1.size)
                println("  Actual Metric after validation: $actualMetric2")
                println("  valid: ${actualMetric2.valid}")
                println("    - support >= MIN_SUPP: ${intersection.size} >= $MIN_SUPP = ${intersection.size >= MIN_SUPP}")
                println("    - confidence > MIN_CONF: ${actualMetric2.confidence} > $MIN_CONF = ${actualMetric2.confidence > MIN_CONF}")
                
                if (actualMetric2.valid) {
                    println("  ✓ Rule SHOULD be created!")
                    
                    // 检查是否在atom2formula2metric中
                    // 规则存储格式：atom2formula2metric[bodyAtom][Formula(headAtom, bodyAtom)]
                    // 所以对于规则 atom2 <= atom1，应该在 atom2formula2metric[atom1] 中查找
                    println("  Checking in atom2formula2metric[bodyAtom=$atom1]...")
                    val formula2metric = atom2formula2metric[atom1]
                    if (formula2metric != null) {
                        println("    Found ${formula2metric.size} formulas with body=$atom1")
                        val existingFormula = formula2metric.keys.find { 
                            it.atom1 == atom2 && it.atom2 == atom1 
                        }
                        if (existingFormula != null) {
                            val storedMetric = formula2metric[existingFormula]!!
                            println("  ✓ Rule EXISTS in atom2formula2metric[bodyAtom]: $storedMetric")
                            ...
                            println("\n  === File Save Analysis ===")
                            println("    Total formulas with body=$atom1: ${allFormulas.size}")
                            println("    High confidence (>=0.6) formulas: ${highConfidenceFormulas.size}")
                            println("    This rule's confidence: ${storedMetric.confidence}")
                            println("    This rule's rank (by confidence): $rank / ${allFormulas.size}")
                            println("    Would be saved to file: ${if (wouldBeSaved) "✓ YES" else "✗ NO (filtered out)"}")
                            ...
                            println("  ✗ Rule NOT FOUND in atom2formula2metric[bodyAtom=$atom1]")
                            println("    Looking for: head=$atom2, body=$atom1")
                            println("    Available formulas (first 5):")
                            formula2metric.entries.take(5).forEach { (f, m) ->
                                println("      head=${f.atom1}, body=${f.atom2}, metric=$m")
                            }
                        ...
                        println("  ✗ No formulas found with body atom $atom1")
                    }
                } else {
                    println("  ✗ Rule would NOT pass final validation")
                }
            ...
            // 分析为什么逆向规则可能没有被创建
            println("\n=== Analyzing inverse rule creation logic ===")
            println("In performLSH, when processing atom2 (as currentAtom):")
            println("  - atom2.isHeadAtom = ${atom2.isHeadAtom}")
            println("  - For inverse rule to be created: metric.inverse().estimateValid && atom2.isHeadAtom")
            println("  - metric.inverse().estimateValid = ${metric2.inverse().estimateValid}")
            println("  - Result: inverse rule creation condition = ${metric2.inverse().estimateValid && atom2.isHeadAtom}")
            
            if (!atom2.isHeadAtom) {
                println("\n⚠️ FOUND THE PROBLEM!")
                println("  atom2 (${atom2}) has isHeadAtom=false")
                println("  This is because:")
                println("    - entityId == getYId(): ${atom2.entityId == IdManager.getYId()}")
                println("    - isInverseRelation: ${IdManager.isInverseRelation(relation2Id)}")
                println("  When atom2 is processed in performLSH:")
                println("    - Forward rule (atom2 <= atom1) would be checked if metric.estimateValid")
                println("    - Inverse rule (atom1 <= atom2) would ONLY be created if atom2.isHeadAtom=true")
                println("  Since atom2.isHeadAtom=false, the inverse rule is NEVER created!")
            }
            
        } else {
            println("\nWARNING: These two atoms do NOT share any buckets!")
            println("This explains why the rule was not discovered.")
            ...
            val estimatedJaccard = matchingHashes.toDouble() / BANDS
            println("\nEstimated Jaccard similarity: $estimatedJaccard (${matchingHashes}/$BANDS matching hashes)")
            
            // 获取实际的instance sets来计算真实的Jaccard
            val instances1 = r2instanceSet[relation1Id]
            val instances2 = r2instanceSet[relation2Id]
            
            if (instances1 != null && instances2 != null) {
                val intersection = instances1.intersect(instances2).size
                val union = instances1.union(instances2).size
                val actualJaccard = if (union > 0) intersection.toDouble() / union else 0.0
                println("Actual Jaccard similarity: $actualJaccard (intersection=$intersection, union=$union)")
                println("Instances1 size: ${instances1.size}, Instances2 size: ${instances2.size}")
            }
        }
    }
